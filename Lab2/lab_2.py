...
def cross_entropy(w, X, y):
    ...
    n = y.shape[0] # number of obeservations to return average HINT from line 74
    ...
    entropy_sum = np.sum((-y * np.log(p)) - ((1-y) * np.log(1-p)))
    # Don't forget to return the average rather than the sum.
    return entropy_sum/n ### Average
...
def question_6():
    return 8.69350877 # <- change this with estimated bias for logit of class=0
...
def question_7():
    return {
      ...
      'Acc': 0.96     # replace with the accuracy for that pair
      }    
